...
for(mote &lt;- sim.allMotes) {
  ...
  for(s &lt;- members) {
    ...
    log(dest_stats, *(&amp;(stats) + index).map(array2Int), mote, s)
  }
}

//val dest_energy = LogWindow("collect-energy.log", List("energy", "mote"))

val dest_energy = sqlitelog.LogTable(sqlitelog.SQLiteDB("coojatrace2.db"), "energy" , List("mote", "energy" ), timeColumn = "Time")
